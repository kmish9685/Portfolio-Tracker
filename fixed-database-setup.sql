-- Fixed Database Setup for Portfolio Dashboard
...
DROP POLICY IF EXISTS "portfolio_holdings_demo_access" ON public.portfolio_holdings;
...
-- Portfolio holdings (user-specific access + demo access)
...
-- Demo access policy for demo user
...
-- 11. Create a demo user in auth.users (if it doesn't exist)
-- Note: This will only work if you have the right permissions
-- If this fails, you can create a user manually through the Supabase Auth UI
...
-- 12. Create demo portfolio holdings (for demo user)
